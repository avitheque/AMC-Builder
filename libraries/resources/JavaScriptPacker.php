 ...
 *
 * ----------------------------------------------------------------------
 * Code adapté pour une exploitation en MVC avec la possibilité de passer
 * un tableau de scripts.
 * # Testé sous PHP 5.4.45
 * @updated		durandcedric@avitheque.net
 * ----------------------------------------------------------------------
 *
...
class JavaScriptPacker {
	...
	public function setEvalWrapper($bBool) {
		$this->_evalWrapper = $bBool;
	}
	...
	private function _basicCompression($script) {
		$parser = new JavaScriptPacker_ParseMaster();
		// make safe
		$parser->escapeChar = '\\';
		// protect strings
		$parser->add('/\'[^\'\\n\\r]*\'/', self::IGNORE);
	...
	private function _bootStrap($packed, $keywords) {
		...
		return sprintf($sFormat, $unpack, $params);
	}
	...
	'function($charCode) {
		return ($charCode < _encoding ? \'\' : arguments.callee($charCode / _encoding)) + String.fromCharCode($charCode % _encoding + 161);
	}';
}
